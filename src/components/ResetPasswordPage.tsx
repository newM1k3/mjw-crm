...
const ResetPasswordPage: React.FC<ResetPasswordPageProps> = ({ onDone }) => {
  ...
  const passwordStrength = (() => {
    ...
    if (password.length < 6) return { level: 'weak', label: 'Too short', color: 'bg-red-400', width: 'w-1/4' };
    if (password.length < 8) return { level: 'fair', label: 'Fair', color: 'bg-orange-400', width: 'w-2/4' };
    const hasUpper = /[A-Z]/.test(password);
    const hasNum = /[0-9]/.test(password);
    const hasSymbol = /[^A-Za-z0-9]/.test(password);
    if (hasUpper && hasNum && hasSymbol) return { level: 'strong', label: 'Strong', color: 'bg-green-500', width: 'w-full' };
    if (hasNum || hasUpper) return { level: 'good', label: 'Good', color: 'bg-green-400', width: 'w-3/4' };
    return { level: 'fair', label: 'Fair', color: 'bg-orange-400', width: 'w-2/4' };
  })();

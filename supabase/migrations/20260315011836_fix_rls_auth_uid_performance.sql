/*
  ...
  ## Summary
  All RLS policies across the application were re-evaluating auth.uid() for every
  single row scanned. Wrapping auth.uid() in a subquery `(select auth.uid())` causes
  Postgres to evaluate it once per query instead of once per row, significantly
  improving performance at scale.

  ## Tables Fixed
  - public.clients (select, insert, update, delete)
  - public.contacts (select, insert, update, delete)
  - public.events (select, insert, update, delete)
  - public.emails (select, insert, update, delete)
  - public.tags (select, insert, update, delete)
  - public.activities (select, insert, update, delete)
  - public.email_templates (select, insert, update, delete)
  - public.user_settings (select, insert, update, delete)

  ## Change
  Replaced `auth.uid()` with `(select auth.uid())` in all USING and WITH CHECK clauses.
*/
